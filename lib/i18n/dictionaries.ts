...
Where facial data is stored: On the organization's NexClock backend servers (the API host configured in the mobile app for that organization), including uploaded attendance photos served through the NexClock API. While the device is offline, the attendance photo needed for that pending clock-in or clock-out is held only in the app's private storage and is deleted automatically after a successful sync. Facial photographs are not written to public storage unless the user explicitly saves a watermarked attendance photo to the photo library.
...
How to revoke consent for facial data: decline or later turn off Camera permission in iOS Settings for NexClock; do not complete face enrollment; reset face enrollment from Profile > Settings; or use Delete Account in Profile. See Your choices and rights for the full deletion path.`,
      },
      {
      ...
      },
      {
        ...
        body: "NexClock does not sell personal data and does not use third-party advertising networks in the mobile app. The mobile app uploads account, attendance, and face data to the organization's NexClock API host configured for that deployment. Camera frames used for pose and liveness guidance are processed on the device by Google ML Kit Face Detection and are not uploaded to Google for identity matching. Face enrollment and attendance identity matching are performed by NexClock's own backend and AI service operated for the organization, not by a commercial third-party cloud facial-recognition API. The optional in-app AI assistant sends the user's typed or spoken prompt to NexClock's AI service so it can answer workforce questions; it is not used to process face photos. If the user enables voice input, Apple speech recognition may convert speech to text on-device or via Apple before the text is sent to NexClock. Firebase Cloud Messaging delivers push notifications and may process a device token; it is not used for advertising or tracking. Hosting and infrastructure providers that store the organization's NexClock servers may process uploaded images and other records only as needed to provide hosting. Those processors, SDKs, and related entities provide the same or equal protection of user data as stated in this Privacy Policy. We obtain permission through the iOS camera prompt, in-app face-enrollment consent, and organization-provisioned account access before facial photographs are collected and uploaded. We obtain separate permission before microphone or speech recognition is used for the AI assistant.",
      },
      {
...
Bagaimana data wajah dihapus: atas permintaan organisasi, proses offboarding akun atau karyawan, reset atau pendaftaran ulang wajah yang didukung produk, permintaan Hapus Akun di Profil, atau sesuai praktik retensi di atas, tunduk pada hukum yang berlaku dan konfigurasi organisasi. Di perangkat, foto enrollment dihapus segera setelah unggahan berhasil atau saat meninggalkan pendaftaran. Foto absensi sementara di penyimpanan privat aplikasi dihapus otomatis setelah sinkronisasi berhasil. Sisa file wajah dihapus saat akun keluar atau diganti.
...
Cara mencabut persetujuan data wajah: tolak atau matikan izin Kamera di Pengaturan iOS untuk NexClock; jangan selesaikan pendaftaran wajah; reset pendaftaran wajah dari Profil > Pengaturan; atau gunakan Hapus Akun di Profil. Lihat Pilihan dan hak Anda untuk jalur penghapusan lengkap.`,
      },
      {
      ...
      },
      {
        ...
        body: "NexClock tidak menjual data pribadi dan tidak memakai jaringan iklan pihak ketiga di aplikasi mobile. Aplikasi mobile mengunggah data akun, kehadiran, dan wajah ke host API NexClock organisasi yang dikonfigurasi untuk deployment tersebut. Frame kamera untuk panduan pose dan liveness diproses di perangkat oleh Google ML Kit Face Detection dan tidak diunggah ke Google untuk pencocokan identitas. Pendaftaran wajah dan pencocokan identitas absensi dilakukan oleh backend dan AI service milik NexClock yang dioperasikan untuk organisasi, bukan API pengenalan wajah cloud pihak ketiga komersial. Asisten AI opsional di aplikasi mengirim prompt ketikan atau suara pengguna ke AI service NexClock untuk menjawab pertanyaan tenaga kerja; tidak dipakai untuk memproses foto wajah. Jika pengguna mengaktifkan input suara, pengenalan ucapan Apple dapat mengubah suara menjadi teks di perangkat atau melalui Apple sebelum teks dikirim ke NexClock. Firebase Cloud Messaging mengirim notifikasi push dan dapat memproses token perangkat; tidak dipakai untuk iklan atau pelacakan. Penyedia hosting dan infrastruktur yang menyimpan server NexClock organisasi dapat memproses gambar dan catatan yang diunggah hanya sejauh diperlukan untuk hosting. Prosesor, SDK, dan entitas terkait tersebut memberikan perlindungan yang sama atau setara terhadap data pengguna sebagaimana dinyatakan dalam Kebijakan Privasi ini. Izin diperoleh melalui prompt kamera iOS, persetujuan pendaftaran wajah di dalam aplikasi, dan akses akun yang disediakan organisasi sebelum foto wajah dikumpulkan dan diunggah. Izin terpisah diminta sebelum mikrofon atau pengenalan ucapan dipakai untuk asisten AI.",
      },
      {
